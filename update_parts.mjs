import fs from 'fs';
const file = 'src/lib/parts-data.ts';
let content = fs.readFileSync(file, 'utf8');
console.log("Read file, length:", content.length);

// 1. Remove generic parts
const startIndex = content.indexOf('    { id: "pi-door-gen-1"');
const endIndex = content.indexOf('    // --- NEW: LENGÉSCSILLAPÍTÓ ÉS RUGÓ ---');
if (startIndex !== -1 && endIndex !== -1) {
    content = content.substring(0, startIndex) + content.substring(endIndex);
    console.log("Successfully removed auto-generated parts.");
} else {
    console.log("Error finding indices for removal.", startIndex, endIndex);
    process.exit(1);
}

// 2. Add new subcategories
const newSubcategories = `    // --- LATEST NEW SUBCATEGORIES ---
...
const marker1 = '    { id: "sc-other-1", categoryId: "other", name: "EGYÉB ALKATRÉSZ", slug: "egyeb-alkatresz" },\\n];';
// Need exact match, let's just do search replace.
const subcatEndIndex = content.indexOf('    { id: "sc-other-1", categoryId: "other", name: "EGYÉB ALKATRÉSZ", slug: "egyeb-alkatresz" },');
if (subcatEndIndex !== -1) {
    const listEnd = content.indexOf('];', subcatEndIndex);
    content = content.substring(0, listEnd) + newSubcategories + content.substring(listEnd);
    console.log("Successfully added new subcategories.");
} else {
    console.log("Could not find marker for subcategories.");
    process.exit(1);
}
...
const partsEndIndex = content.lastIndexOf('];');
if (partsEndIndex !== -1) {
    content = content.substring(0, partsEndIndex) + newParts + '];\\n';
    console.log("Successfully added new parts.");
} else {
    console.log("Could not find end of parts array.");
    process.exit(1);
}
