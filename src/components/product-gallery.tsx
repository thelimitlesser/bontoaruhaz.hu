            ...
            */}
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/20 pointer-events-none">
                    <div className="bg-black/60 backdrop-blur-md rounded-full p-4 border border-white/10 shadow-2xl">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none">
                    {images.map((img, idx) => (
                        <button
                            ...
                            className={clsx("relative w-24 h-24 shrink-0 overflow-hidden transition-all duration-300",
                                selectedImage === img
                                    ?"ring-2 ring-[var(--color-primary)] ring-offset-2 ring-offset-background scale-105 shadow-[0_0_15px_rgba(var(--color-primary-rgb, 219,81,60),0.5)]" :"opacity-60 hover:opacity-100 hover:scale-105" )}
                            style={{ borderRadius:'20px' }}
                        >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                                src={img}
                                className="object-cover w-full h-full" alt={`${productName} - kép ${idx + 1}`}
                            />
                        </button>
                    ))}
                </div>
