import { ArrowLeft, ShoppingCart, ShieldCheck, Truck, HelpCircle, Star, Info, Globe, Calendar, CheckCircle2, Factory, Hash, Settings, Phone, Mail } from"lucide-react";
            ...
            <div className="bg-foreground/[0.02] border border-border rounded-3xl overflow-hidden shadow-sm">
              ...
              <div className="p-6 md:p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-background rounded-2xl border border-border flex items-start gap-3">
                    <Star className="w-5 h-5 text-muted mt-0.5" />
                    <div>
                      <div className="text-xs text-muted uppercase tracking-wider font-bold">Állapot</div>
                      <div className="font-mono text-lg font-bold text-foreground mt-0.5">{product.condition ||"Ismeretlen"}</div>
                    </div>
                  </div>

                  {/* Year / Universal block */}
                  <div className="p-4 bg-background rounded-2xl border border-border flex items-start gap-3">
                    {dbPart?.isUniversal ? <Globe className="w-5 h-5 text-muted mt-0.5" /> : <Calendar className="w-5 h-5 text-muted mt-0.5" />}
                    <div>
                      <div className="text-xs text-muted uppercase tracking-wider font-bold">{dbPart?.isUniversal ?"Típus" :"Évjárat"}</div>
                      <div className="font-mono text-lg font-bold text-foreground mt-0.5">
                        {dbPart?.isUniversal ?"Univerzális Alkatrész" : (yearString ||"-")}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-background rounded-2xl border border-border flex items-start gap-3">
                    ...
                    <div>
                      <div className="text-xs text-muted uppercase tracking-wider font-bold">Belső Termékszám</div>
                      <div className="font-mono text-lg font-bold text-foreground mt-0.5">{dbPart?.productCode ||"-"}</div>
                    </div>
                  </div>
                  {product.sku && (
                    <div className="p-4 bg-background rounded-2xl border border-border flex items-start gap-3">
                      <Factory className="w-5 h-5 text-muted mt-0.5" />
                      <div>
                        <div className="text-xs text-muted uppercase tracking-wider font-bold">Gyári Cikkszám (OEM)</div>
                        <div className="font-mono text-lg font-bold text-foreground mt-0.5">{product.sku}</div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="sticky top-32 space-y-6">
              ...
              <div className="bg-background border-2 border-border rounded-[2rem] p-6 md:p-8 shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative z-10 space-y-6">
                  {/* Price Block */}
                  <div>
                    <span className="text-xs font-bold text-muted uppercase tracking-widest block mb-1">Bruttó fogyasztói ár</span>
          ...
          <div className="glass-panel rounded-[2rem] p-6 md:p-10 border-2 border-[var(--color-primary)]/20 relative overflow-hidden group max-w-4xl mx-auto">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <p className="text-base text-muted leading-relaxed max-w-2xl mb-8">
                Szakértő kollégáink szívesen segítenek a beazonosításban telefonon vagy e-mailben is.
              </p>

              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <a
                  href="tel:+36301234567" className="flex items-center justify-center gap-2.5 px-6 py-3 bg-foreground text-background font-bold text-sm rounded-xl hover:scale-105 transition-all shadow-lg" >
                  <Phone className="w-4 h-4" />
                  +36 30 123 4567
                </a>
                <Link
                  href={`/contact?subject=${encodeURIComponent(`Kérdés a(z) ${product.sku} cikkszámú termékkel kapcsolatban`)}`}
                  className="flex items-center justify-center gap-2.5 px-6 py-3 bg-[var(--color-primary)] text-white font-bold text-sm rounded-xl hover:scale-105 transition-all shadow-lg shadow-[var(--color-primary)]/20" >
