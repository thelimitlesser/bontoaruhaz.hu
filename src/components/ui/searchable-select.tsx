    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            <button
                ...
                disabled={disabled}
                className={clsx(
                    "w-full text-left rounded-lg px-4 py-3 flex items-center justify-between transition-all outline-none border", // changed rounded-xl to rounded-lg to match other admin inputs
                    disabled ? "opacity-50 cursor-not-allowed border-transparent text-muted" :
                        theme === "dark" ? "bg-foreground/5 border-border text-foreground hover:border-[var(--color-primary)]/50 focus:border-[var(--color-primary)]" :
                            "bg-gray-50 border-gray-200 text-gray-900 hover:border-[var(--color-primary)]/50 focus:border-[var(--color-primary)]",
            ...
            {isOpen && (
                <div className={clsx("absolute z-50 w-full mt-2 border rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-100 origin-top",
                    ...
                    <div className={clsx("p-2 border-b sticky top-0 z-10", theme === "dark" ? "border-border bg-background/95" : "border-gray-200 bg-white")}>
                        <div className="relative">
                            ...
                            <input
                                ref={inputRef}
                                type="text"
                                className={clsx("w-full border border-transparent rounded-lg pl-9 pr-3 py-2 text-sm focus:outline-none focus:border-[var(--color-primary)] transition-colors",
                    ...
                    {/* Options List */}
                    <div className={clsx("max-h-[140px] overflow-y-auto scrollbar-thin scrollbar-track-transparent", theme === "dark" ? "scrollbar-thumb-foreground/20" : "scrollbar-thumb-gray-200")}>
                                    ...
                                    <li key={option.value}>
                                        <button
                                            ...
                                            className={clsx(
                                                ...
                                                value === option.value
                                                    ? theme === "dark" ? "bg-[var(--color-primary)]/10 text-[var(--color-primary)]" : "bg-[var(--color-primary)]/10 text-[var(--color-primary)] font-bold"
                                                    : theme === "dark" ? "text-foreground hover:bg-foreground/5 hover:text-[var(--color-primary)]" : "text-gray-700 hover:bg-gray-50 hover:text-[var(--color-primary)]"
                                            )}
                                            onClick={() => {
